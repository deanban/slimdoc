...
import { MARGIN_SHARE } from './pdf-furniture.js';
import { sanitizeText } from './utils/text.js';

export { suppressRunningText } from './pdf-furniture.js';
...
const TIGHT_RUN = 0.3;
/**
 * A run starting this many characters *before* the previous one's end is
 * geometry left-to-right layout cannot explain — not kerning overhang. Joining
 * the two would invent a token (`9,942` + `2` → `9,9422`), so it gets a space:
 * the failure that adds a boundary instead of the one that fabricates a number.
 */
const BACKWARDS_OVERLAP = 1;
...
function layOut(items: TextItem[], origin: number, charWidth: number): string {
  ...
  for (const item of items) {
    ...
    else if (previousRight !== undefined) {
      const gap = item.x - previousRight;
      if (gap > charWidth * TIGHT_RUN || gap < -charWidth * BACKWARDS_OVERLAP) out += ' ';
    }
