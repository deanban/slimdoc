...
test('teams: collapses the five-line Word block into one turn', () => {
  ...
  assert.equal(
    out,
    [
      'Adaeze [0:43]: Hey, everyone, good afternoon.',
      'Tobias [0:47]: Afternoon.',
      'Adaeze [1:05]: Let\'s start with the release checklist. I put the draft in the shared folder yesterday.',
      'Renée [1:02:03]: We still owe the localisation team a build. I can cut one tomorrow morning if nobody objects.',
...
test('teams: the spec target shape, with full names', () => {
  const block = [
    ...
    '',
    'Mendoza, Renata 0 minutes 43 seconds',
    '',
    'Hey, everyone, good afternoon\\.',
  ].join('\n');
  assert.equal(
    tidyTranscript(block, { shortenNames: false }),
    'Mendoza, Renata [0:43]: Hey, everyone, good afternoon.',
  );
});
...
test('teams: singular units, plural units and an hours component', () => {
  const out = tidyTranscript(
    [
      'Vance, Iris 1 minute 5 seconds1:05',
      'Vance, Iris 1 minute 5 seconds',
      'One.',
      '',
      'Vance, Iris 2 minutes 0 seconds',
      'Two.',
      '',
      'Vance, Iris 1 hour 0 minutes 7 seconds',
      'Three.',
      '',
      'Vance, Iris 2 hours 15 minutes 42 seconds',
      'Four.',
    ].join('\n'),
...
test('teams: a continuation turn without avatar or bold line still merges', () => {
  const out = tidyTranscript(TEAMS);
  assert.match(out, /Adaeze \[1:05\]: Let's start with the release checklist\. I put the draft/);
  assert.equal(out.match(/Adaeze \[/g).length, 2);
});

test('teams: the AI-generated banner is dropped, escaped or not', () => {
  assert.ok(!tidyTranscript(TEAMS).includes('AI-generated'));
  const plain = 'AI-generated content may be incorrect\n\nHale, Otto 0 minutes 3 seconds\n\nHello.';
  assert.equal(tidyTranscript(plain), 'Otto [0:03]: Hello.');
});
...
test('avatar leftovers, icon glyphs and initials lines are dropped', () => {
  const out = tidyTranscript(
    ['', '[IMG]', '![](avatar.png)', 'DB', 'Ruiz, Marta 0 minutes 5 seconds', 'Hello.'].join('\n'),
  );
  assert.equal(out, 'Marta [0:05]: Hello.');
});
...
test('webvtt: voice spans, cue indices, NOTE blocks and duplicate cues', () => {
  assert.equal(
    tidyTranscript(VTT),
    [
      'Dana [0:01]: Morning all, thanks for joining.',
      'Priya [0:04]: Can everyone see the board?',
      'Dana [1:12]: Yes, looks fine here.',
    ].join('\n\n'),
  );
});
...
test('srt: numeric cue indices and comma timings', () => {
  assert.equal(
    tidyTranscript(SRT),
    [
      "Dana [0:02]: Let's pick this up where we left off.",
      'Priya [0:05]: Agreed, I have the notes open. The first item is the migration window.',
    ].join('\n\n'),
  );
});
...
test('zoom / meet / otter line shapes', () => {
  assert.equal(
    tidyTranscript(ZOOM),
    [
      'Dana [0:12]: Morning all, thanks for joining.',
      'Priya [12:04:31]: Can everyone see the board?',
      'Dana [12:44]: Yes, looks fine here.',
      'Marcus [13:02]: I lost audio for a second there, but I am back now.',
    ].join('\n\n'),
  );
});
...
test('a short line that merely starts like the next one is kept', () => {
  const out = tidyTranscript('Kerr, Sam 0 minutes 5 seconds\nSo.\n\nKerr, Sam 0 minutes 9 seconds\nSo we should ship it.');
  assert.equal(out, 'Sam [0:05]: So. So we should ship it.');
});
...
test('system lines are dropped as whole lines only', () => {
  const input = [
    'Recording started',
    'Nguyen, Bao joined the meeting',
    'Transcription started by Nguyen, Bao',
    'Nguyen, Bao is presenting',
    'Muted',
    'This transcript was generated automatically and may contain errors.',
    '',
    'Nguyen, Bao 0 minutes 8 seconds',
  ...
  assert.equal(
    ...
    'Bao [0:08]: I see some news has joined the discussion, so let us wait a moment.',
  );
});
...
test('shortenNames handles "Last, First", accents and double surnames', () => {
  assert.ok(tidyTranscript(TEAMS).includes('Renée [1:02:03]:'));
});

test('shortenNames bails out to full names when first names collide', () => {
  const out = tidyTranscript(
    'Smith, Dana 0 minutes 5 seconds\nHello.\n\nJones, Dana 0 minutes 9 seconds\nHi there.',
  );
  assert.equal(out, 'Smith, Dana [0:05]: Hello.\n\nJones, Dana [0:09]: Hi there.');
});

test('shortenNames: false keeps every full name', () => {
  const out = tidyTranscript(TEAMS, { shortenNames: false });
  assert.ok(out.startsWith('Okonkwo, Adaeze [0:43]:'));
  assert.ok(out.includes('Ferreira Duarte, Renée [1:02:03]:'));
});

test('"First Last" shortens to the first name', () => {
  assert.ok(tidyTranscript(ZOOM).startsWith('Dana [0:12]:'));
});
...
test('keepFirstTimestamp: false removes every timestamp', () => {
  ...
  assert.ok(out.startsWith('Adaeze: Hey, everyone, good afternoon.'));
...
test('dropTimestamps: false keeps a stamp per utterance', () => {
  ...
  assert.ok(out.includes('Adaeze: [1:05] Let\'s start with the release checklist. [1:20] I put the draft'));
});
...
test('a block over 200 chars puts its text on its own line', () => {
  ...
  assert.equal(tidyTranscript(`Poe, Ann 0 minutes 1 second\n${short}`), `Ann [0:01]: ${short}`);
  assert.equal(tidyTranscript(`Poe, Ann 0 minutes 1 second\n${long}`), `Ann [0:01]:\n${long}`);
});
